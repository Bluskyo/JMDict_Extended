...
# for csv format change name to csv and 
# uncomment last lines in loop.

waduDict = {}

with open('data/wadoku_pitchdb.json', 'w') as f:
    for entry in entries:
        ...
        for orth in clean_orth(orths_txt).split("␟"):
            waduDict[orth] = {
                "orths" : orth, 
                "hira" : hira_txt.split("␟"), 
                "hatsuon" : hatsuon_txt.split("␟"), 
                "acc_patts" : acc_patts_txt.split("␟"), 
                "zo_patts" : zo_patts_txt.split("␟")
            }

    f.write(json.dumps(waduDict, ensure_ascii=False))